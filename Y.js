...
.z-nav-label {
    ...
    // ================================================================================================================
    // MODULE 24: EXTENDED CSS INJECTION (LYRICS & SETTINGS VIEWS)
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 8 (Line ~2716)
    class ExtendedCSSInjector {
        static inject() {
            const logger = window.ZENITH_CORE.Logger;
            logger.info('ExtendedCSSInjector', 'Injecting Lyrics and Settings Stylesheet...');

            const style = document.createElement('style');
            style.id = 'zenith-extended-styles';
            style.textContent = `
/* ==========================================================================
   LYRICS VIEW
   ========================================================================== */
.z-lyrics-container {
    ...
    overflow-y: auto;
    padding: 24px;
    padding-bottom: 120px; /* Space for transport controls */
...
.z-lyrics-container::-webkit-scrollbar {
    display: none;
}
...
.z-spinner {
    ...
    border-top: 4px solid var(--z-text-primary);
    border-radius: 50%;
...
.z-setting-row-vertical {
    display: flex;
    flex-direction: column;
    gap: 12px;
}

.z-setting-info-header {
    display: flex;
    justify-content: space-between;
    align-items: center;
}
...
.z-version-string {
    ...
    margin-top: 16px;
            ...
            `;
            document.head.appendChild(style);
            logger.info('ExtendedCSSInjector', 'Lyrics and Settings Stylesheet injected successfully.');
        }
    }
    window.ZENITH_CORE.ExtendedCSSInjector = ExtendedCSSInjector;

    // Trigger Extended CSS Injection immediately
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', ExtendedCSSInjector.inject);
    } else {
        ExtendedCSSInjector.inject();
    }
    // LINE COUNT TRACKER: FINAL FOR PART 8 (Line ~3048)
    // ================================================================================================================
    // MODULE 25: THE NETWORK INTERCEPTOR (STREAM SNIFFER)
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 9 (Line ~3049)
    class NetworkInterceptor {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.originalFetch = window.fetch;
            this.originalXHR = window.XMLHttpRequest;
            this.interceptedStreams = new Map();
            
            this.initFetchInterceptor();
            this.initXHRInterceptor();
            this.logger.info('NetworkInterceptor', 'Network layer successfully hijacked.');
        }
        ...
        initFetchInterceptor() {
            ...
            window.fetch = async function(...args) {
                ...
                if (url && url.includes('videoplayback') && url.includes('mime=audio')) {
                    self.logger.info('NetworkInterceptor', 'Raw audio stream intercepted via Fetch');
                    self.processStreamURL(url);
                }

                if (url && url.includes('youtubei/v1/player')) {
                    // This endpoint contains the raw player response, including streaming data
                    self.logger.info('NetworkInterceptor', 'Player configuration metadata intercepted.');
                }
                ...
                xhr.open = function(method, url, ...rest) {
                    if (typeof url === 'string') {
                        if (url.includes('videoplayback') && url.includes('mime=audio')) {
                            self.logger.info('NetworkInterceptor', 'Raw audio stream intercepted via XHR');
                            self.processStreamURL(url);
                        }
                    }
        ...
        processStreamURL(url) {
            try {
                ...
                // Track intercepted streams by itag (quality format code)
                this.interceptedStreams.set(itag, url);
                
                window.ZENITH_CORE.store.dispatch({
                    type: 'UPDATE_STREAM_META',
                    payload: { 
                ...
                this.logger.warn('NetworkInterceptor', 'Failed to parse stream URL', e);
            }
        }
    }
    window.ZENITH_CORE.NetworkInterceptor = NetworkInterceptor;
    // LINE COUNT TRACKER: CURRENT ~3130

    // ================================================================================================================
    // MODULE 26: WEB AUDIO API VISUALIZER ENGINE
    // ================================================================================================================
    class AudioVisualizerEngine {
        constructor() {
            ...
            // Subscriptions to visualizer data
            this.subscribers = new Set();
        }
        ...
        async initialize(mediaElement) {
            ...
            try {
                ...
                // High resolution for detailed visualizers (fftSize must be a power of 2)
                this.analyser.fftSize = 2048; 
                this.analyser.smoothingTimeConstant = 0.85;

                // Create cross-origin media source
                ...
                // Allocate data arrays based on analyser bin count
                const bufferLength = this.analyser.frequencyBinCount;
                this.frequencyData = new Uint8Array(bufferLength);
                this.timeDomainData = new Uint8Array(bufferLength);

                this.isInitialized = true;
                this.logger.info('AudioVisualizer', 'Web Audio API successfully hooked to physical media element.');

                // Resume context if suspended (Browser autoplay policy mitigation)
                ...
                this.logger.error('AudioVisualizer', 'Failed to initialize Web Audio API', err);
            }
        }
        ...
        startRenderLoop() {
            const loop = () => {
                if (this.isInitialized && this.subscribers.size > 0) {
                    // Pull raw byte data from the DSP node
                    this.analyser.getByteFrequencyData(this.frequencyData);
                    this.analyser.getByteTimeDomainData(this.timeDomainData);

                    // Broadcast raw buffer data to all connected canvas renderers
    ...
    window.ZENITH_CORE.AudioVisualizerEngine = AudioVisualizerEngine;
    // LINE COUNT TRACKER: FINAL FOR PART 9 (Line ~3232)
    // ================================================================================================================
    // MODULE 27: CANVAS VISUALIZER RENDERER (UI LAYER)
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 10 (Line ~3233)
    class CanvasRenderer {
        ...
        mount() {
            ...
            if (!this.canvas) {
                this.logger.warn('CanvasRenderer', `Canvas element #${this.canvasId} not found.`);
                return false;
            }
            ...
            this.logger.info('CanvasRenderer', 'Canvas mounted and context acquired.');
            return true;
        }
        ...
        handleResize() {
            ...
            // Set actual size in memory (scaled to account for retina/high-DPI displays)
        ...
        connectEngine(engine) {
            ...
            this.unsubscribe = engine.subscribe((data) => this.renderFrame(data));
        }
        ...
        renderFrame(data) {
            ...
            for (let i = 0; i < bufferLength; i++) {
                const barHeight = (frequency[i] / 255) * this.height;

                // Create a gradient for each bar based on the dominant theme color
                const gradient = this.ctx.createLinearGradient(0, this.height, 0, this.height - barHeight);
                gradient.addColorStop(0, `${this.themeColor}33`); // 20% opacity
                gradient.addColorStop(1, this.themeColor);

                this.ctx.fillStyle = gradient;
                
                // Draw rounded rects for a modern feel
                const radius = barWidth / 2;
                const h = Math.max(barHeight, radius * 2); // Minimum height to allow border radius
                ...
                this.ctx.lineTo(x + barWidth, y + h);
    ...
    window.ZENITH_CORE.CanvasRenderer = CanvasRenderer;
    // LINE COUNT TRACKER: CURRENT ~3315

    // ================================================================================================================
    // MODULE 28: SMART SLEEP TIMER & FADER
    // ================================================================================================================
    class SleepTimer {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            ...
            this.timerId = null;
            this.fadeDurationMs = 30000; // 30 seconds to fade out completely
            this.isFading = false;
        }

        /**
         * Sets the sleep timer
         * @param {number} minutes - Minutes until playback stops
         */
        set(minutes) {
            this.clear();
            const now = Date.now();
            this.targetTime = now + (minutes * 60 * 1000);
            
            this.logger.info('SleepTimer', `Timer set for ${minutes} minutes.`);
            
            this.timerId = setInterval(() => this.check(), 1000);
            
            window.ZENITH_CORE.store.dispatch({
                type: 'UPDATE_SLEEP_TIMER',
                payload: { active: true, targetTime: this.targetTime }
            });
        }
        ...
        clear() {
            ...
            window.ZENITH_CORE.store.dispatch({
                type: 'UPDATE_SLEEP_TIMER',
                payload: { active: false, targetTime: null }
            });
        }

        check() {
            if (!this.targetTime || this.isFading) return;

            const timeRemaining = this.targetTime - Date.now();

            if (timeRemaining <= this.fadeDurationMs) {
                this.logger.info('SleepTimer', 'Threshold reached. Initiating volume fade sequence.');
        ...
        initiateFadeOut() {
            this.isFading = true;
            const broker = window.ZENITH_CORE.MediaBroker;
            if (!broker.videoElement) {
                this.executeStop();
                return;
            }

            const startVolume = broker.videoElement.volume;
            const steps = 30; // 30 steps for the fade
            const stepDuration = this.fadeDurationMs / steps;
            ...
            const fadeInterval = setInterval(() => {
                ...
                // Easing curve (ease-out) for more natural audio decay
                broker.videoElement.volume = Math.pow(newVolume, 2);

                if (currentStep >= steps) {
                    clearInterval(fadeInterval);
                    this.executeStop();
                    // Restore volume after pausing so it's ready for next time
                    setTimeout(() => {
                        broker.videoElement.volume = startVolume;
                    }, 500);
                }
            }, stepDuration);
        }
        ...
        executeStop() {
            this.logger.info('SleepTimer', 'Executing pause command.');
            const broker = window.ZENITH_CORE.MediaBroker;
            broker.pause();
            this.clear();
        }
    }
    window.ZENITH_CORE.SleepTimer = SleepTimer;
    // LINE COUNT TRACKER: FINAL FOR PART 10 (Line ~3412)
    // ================================================================================================================
    // MODULE 29: PLAYLIST MANAGER (QUEUE & PLAYBACK STATE)
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 11 (Line ~3413)
    class PlaylistManager {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.queue = [];
            this.originalQueue = []; // Retained for un-shuffling
            this.currentIndex = -1;
        ...
        loadQueue(trackList, startIndex = 0) {
            if (!Array.isArray(trackList) || trackList.length === 0) {
                this.logger.warn('PlaylistManager', 'Attempted to load an invalid or empty queue.');
                return;
            }
            ...
            if (this.isShuffled) {
                this.applyShuffle(true); // Keep current track at index 0
            }

            this.logger.info('PlaylistManager', `Queue loaded with ${this.queue.length} tracks.`);
        ...
        next() {
            ...
            if (nextTrack) {
                if (this.repeatMode !== 2) {
                    this.currentIndex = this.queue.indexOf(nextTrack);
                }
                this.logger.info('PlaylistManager', `Advancing to next track: ${nextTrack.videoId}`);
                this.emitQueueUpdate();
                return nextTrack;
            }
            this.logger.info('PlaylistManager', 'End of queue reached.');
            return null;
        }
        ...
        prev() {
            ...
            // If repeat one is on, or we are past 3 seconds, just restart current track
            // (Handling the 3-second rule is usually done at the player level, but we provide the raw index shift here)
            ...
            if (prevIndex < 0) {
                if (this.repeatMode === 1) {
                    ...
                    prevIndex = 0; // Stick to the first track
                }
            }

            this.currentIndex = prevIndex;
            this.logger.info('PlaylistManager', `Returning to previous track: ${this.queue[this.currentIndex].videoId}`);
            this.emitQueueUpdate();
            return this.queue[this.currentIndex];
        }
        ...
        toggleShuffle() {
            ...
            this.logger.info('PlaylistManager', `Shuffle mode set to: ${this.isShuffled}`);
            this.emitQueueUpdate();
        }

        applyShuffle(keepCurrentFirst = false) {
            ...
            if (keepCurrentFirst && currentTrack) {
                // Find where the current track ended up and swap it to index 0
        ...
        restoreOriginalQueue() {
            ...
            if (currentTrack) {
                // Find where the current track is in the original queue to maintain sync
                const newIndex = this.queue.findIndex(t => t.videoId === currentTrack.videoId);
        ...
        cycleRepeatMode() {
            ...
            this.logger.info('PlaylistManager', `Repeat mode cycled to: ${modes[this.repeatMode]}`);
            this.emitQueueUpdate();
    ...
    window.ZENITH_CORE.PlaylistManager = PlaylistManager;
    // LINE COUNT TRACKER: CURRENT ~3550

    // ================================================================================================================
    // MODULE 30: GAPLESS PRE-FETCHER (BACKGROUND AUDIO BUFFERING)
    // ================================================================================================================
    class GaplessPreFetcher {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.activePrefetchUrl = null;
            this.audioCache = new Map(); // Store array buffers
            this.maxCacheSize = 2; // Only hold the next 2 tracks in memory
            this.isPrefetching = false;
        }

        /**
         * Initiates a background fetch for the next track to eliminate network latency between songs.
         * @param {string} videoId - The YouTube Video ID of the upcoming track.
         */
        async initiatePrefetch(videoId) {
            if (this.isPrefetching) return;
            if (this.audioCache.has(videoId)) {
                this.logger.info('PreFetcher', `Track ${videoId} already in cache. Skipping prefetch.`);
                return;
            }

            this.logger.info('PreFetcher', `Starting gapless prefetch for upcoming track: ${videoId}`);
            this.isPrefetching = true;
            this.activePrefetchUrl = videoId;

            try {
                // 1. We must mock the YTM API call to get the streaming URL for the NEXT track
                // Note: This relies on the internal API interceptors we built earlier.
                const streamData = await this.resolveStreamUrl(videoId);
                
                if (!streamData || !streamData.url) {
                    throw new Error('Failed to resolve stream URL for prefetch.');
                }

                // 2. Fetch the actual audio binary data
                const response = await fetch(streamData.url, {
                    method: 'GET',
                    headers: { 'Range': 'bytes=0-1000000' } // Fetch first 1MB to ensure immediate playback start
                });

                if (response.ok) {
                    const buffer = await response.arrayBuffer();
                    this.cacheAudioBuffer(videoId, buffer);
                    this.logger.info('PreFetcher', `Successfully buffered first chunk for ${videoId}`);
                } else {
                    throw new Error(`HTTP ${response.status} during prefetch.`);
                }
            } catch (err) {
                this.logger.error('PreFetcher', `Prefetch failed for ${videoId}`, err);
            } finally {
                this.isPrefetching = false;
                this.activePrefetchUrl = null;
            }
        ...
        async resolveStreamUrl(videoId) {
            // In a live environment, this communicates with the Zenith Network Interceptor
            // or constructs a pristine YouTubei player request.
            // For architecture completeness, we simulate the async resolution.
            return new Promise(resolve => {
                setTimeout(() => {
                    resolve({ url: `https://googlevideo.com/videoplayback?id=${videoId}&itag=140&mock=true` });
                }, 500);
            });
        }

        cacheAudioBuffer(videoId, arrayBuffer) {
            // Manage cache size limits
            if (this.audioCache.size >= this.maxCacheSize) {
                const firstKey = this.audioCache.keys().next().value;
                this.audioCache.delete(firstKey);
                this.logger.info('PreFetcher', `Evicted ${firstKey} from cache to maintain size limits.`);
            }
            this.audioCache.set(videoId, arrayBuffer);
        }

        getPrefetchedBuffer(videoId) {
            const buffer = this.audioCache.get(videoId);
            if (buffer) {
                this.logger.info('PreFetcher', `Cache hit for ${videoId}. Retrieving audio buffer.`);
                return buffer;
            }
            return null;
        }
        ...
        clearCache() {
            ...
            this.logger.info('PreFetcher', 'Audio buffer cache cleared.');
        }
    }
    window.ZENITH_CORE.GaplessPreFetcher = GaplessPreFetcher;
    // LINE COUNT TRACKER: FINAL FOR PART 11 (Line ~3637)
    // ================================================================================================================
    // MODULE 31: ADVANCED LYRICS SYNC ENGINE
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 12 (Line ~3638)
    class LyricsSyncEngine {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.lyricsData = []; // Array of { time: float, text: string }
            this.currentIndex = -1;
            this.containerSelector = '.z-lyrics-container';
            this.lineSelectorPrefix = 'z-lyric-line-';
            this.isScrubbing = false; // Prevents auto-scroll if user is manually scrolling
            this.scrollTimeout = null;
        }

        /**
         * Parses a raw LRC formatted string into an array of timed objects.
         * Handles multiple timestamps for repeated chorus lines: [00:12.00][01:22.00] Chorus
         */
        parseLRC(lrcString) {
            this.logger.info('LyricsEngine', 'Parsing LRC payload...');
            this.lyricsData = [];
            
            if (!lrcString || typeof lrcString !== 'string') {
                this.logger.warn('LyricsEngine', 'Invalid LRC payload provided.');
                return false;
            }
            ...
            for (const line of lines) {
                ...
                let match;
                // A single text line might have multiple timestamp tags
                while ((match = timeRegex.exec(line)) !== null) {
                    const minutes = parseInt(match[1], 10);
                    const seconds = parseInt(match[2], 10);
                    const milliseconds = parseInt(match[3].padEnd(3, '0'), 10); // Handle .xx and .xxx
            ...
            // Sort chronologically (necessary because of multi-timestamp parsing)
            this.lyricsData.sort((a, b) => a.time - b.time);
            
            this.logger.info('LyricsEngine', `Successfully parsed ${this.lyricsData.length} lyric events.`);
            return true;
        }

        /**
         * Updates the active lyric based on the current media playback time.
         * @param {number} currentTime - Current audio position in seconds.
         */
        sync(currentTime) {
            if (this.lyricsData.length === 0) return;

            // Find the highest index where the time is less than or equal to current time
            let newIndex = -1;
            ...
            // Only update DOM if the active line has changed
        ...
        updateDOM() {
            ...
            // Grab all rendered lyric lines
            const lines = container.querySelectorAll('.z-lyric-line');
            if (lines.length === 0) return;

            lines.forEach((lineEl, index) => {
                if (index < this.currentIndex) {
                    lineEl.className = 'z-lyric-line passed';
                } else if (index === this.currentIndex) {
                    lineEl.className = 'z-lyric-line active';
                    this.scrollToActive(container, lineEl);
                } else {
                    lineEl.className = 'z-lyric-line';
                }
            });
        }

        scrollToActive(container, activeElement) {
            ...
            // We want the active line to sit exactly in the middle of the viewport
        ...
        // Suspend auto-scrolling when user interacts with the lyrics view
        suspendAutoScroll() {
            ...
            this.scrollTimeout = setTimeout(() => {
                ...
                this.updateDOM(); // Snap back to current
            }, 3000);
        }
    ...
    window.ZENITH_CORE.LyricsSyncEngine = LyricsSyncEngine;
    // LINE COUNT TRACKER: CURRENT ~3755

    // ================================================================================================================
    // MODULE 32: LOCAL STORAGE PERSISTENCE LAYER
    // ================================================================================================================
    class StorageAdapter {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.prefix = 'ZENITH_DATA_';
            this.memoryCache = new Map(); // Fallback for incognito mode quota limits
        }

        /**
         * Save key-value data to localStorage
         */
        save(key, value) {
            ...
            try {
                ...
                // Handle QuotaExceededError (Incognito mode or full disk)
                this.logger.warn('StorageAdapter', `Storage quota exceeded or restricted. Falling back to memory cache for ${key}.`);
        ...
        load(key, defaultValue = null) {
            ...
            try {
                ...
                this.logger.warn('StorageAdapter', `Failed to read from localStorage. Checking memory cache for ${key}.`);
            }

            if (!serialized) {
                serialized = this.memoryCache.get(fullKey);
            }
            ...
            if (serialized) {
                try {
                    ...
                    this.logger.error('StorageAdapter', `Corrupted JSON payload in storage for ${key}`, parseError);
                    return defaultValue;
                }
            }
            return defaultValue;
        ...
        /**
         * Create a full JSON backup of all Zenith settings
         */
        exportData() {
            const exportPayload = {};
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (key.startsWith(this.prefix)) {
                    exportPayload[key.replace(this.prefix, '')] = JSON.parse(localStorage.getItem(key));
                }
            }
        ...
        /**
         * Restore settings from a JSON backup payload
         */
        importData(jsonString) {
            try {
                ...
                this.logger.info('StorageAdapter', 'Data successfully imported.');
                return true;
            } catch (e) {
                this.logger.error('StorageAdapter', 'Failed to import data: Invalid JSON structure.', e);
                return false;
            }
        }
    }
    window.ZENITH_CORE.StorageAdapter = StorageAdapter;
    // LINE COUNT TRACKER: FINAL FOR PART 12 (Line ~3848)
    // ================================================================================================================
    // MODULE 33: MEDIA SESSION EVENT BRIDGE (HARDWARE KEYS & HEADSETS)
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 13 (Line ~3849)
    class MediaSessionBridge {
        constructor() {
            ...
            if (this.isSupported) {
                this.initHandlers();
                this.logger.info('MediaSessionBridge', 'Hardware media keys and headset events hooked.');
            } else {
                this.logger.warn('MediaSessionBridge', 'Media Session API not supported in this browser.');
            }
        }
        ...
        initHandlers() {
            ...
            const actionHandlers = {
                ...
                pause: () => broker.pause(),
                previoustrack: () => {
                    const pm = window.ZENITH_CORE.PlaylistManager;
                    if (pm) pm.prev();
                },
                nexttrack: () => {
                    const pm = window.ZENITH_CORE.PlaylistManager;
                ...
                seekbackward: (details) => {
                    ...
                    broker.seekTo(broker.getCurrentTime() - skipTime);
                },
                seekforward: (details) => {
                    const skipTime = details.seekOffset || 10;
                    broker.seekTo(broker.getCurrentTime() + skipTime);
                ...
                seekto: (details) => {
                    if (details.fastSeek && 'fastSeek' in broker.videoElement) {
                        broker.videoElement.fastSeek(details.seekTime);
                    } else {
                        broker.seekTo(details.seekTime);
                    }
                }
            };
            ...
            for (const [action, handler] of Object.entries(actionHandlers)) {
                try {
                    ...
                    this.logger.warn('MediaSessionBridge', `Action '${action}' is not supported.`, error);
                }
            }
        }

        /**
         * Updates OS-level media notifications (Lock screen, Android auto, Windows media flyout)
         * @param {Object} track - The currently playing track metadata object
         */
        updateMetadata(track) {
            if (!this.isSupported || !track) return;

            // Extract high-res artwork if available, fallback to default
            const artwork = track.thumbnails ? track.thumbnails.map(t => ({
                src: t.url,
            ...
            this.logger.info('MediaSessionBridge', `OS Media Notification updated for: ${track.title}`);
        }

        /**
         * Updates the OS playback state (Play/Pause icon)
    ...
    window.ZENITH_CORE.MediaSessionBridge = MediaSessionBridge;
    // LINE COUNT TRACKER: CURRENT ~3920

    // ================================================================================================================
    // MODULE 34: KEYBOARD SHORTCUTS MANAGER
    // ================================================================================================================
    class ShortcutManager {
        constructor() {
            ...
            this.bindEvents();
            this.logger.info('ShortcutManager', 'Keyboard listeners active.');
        }

        registerDefaultShortcuts() {
            // Broker mapping
            this.bindings.set('Space', { 
                action: () => window.ZENITH_CORE.MediaBroker.togglePlay(), 
                description: 'Play/Pause' 
            });
            this.bindings.set('KeyM', { 
                action: () => window.ZENITH_CORE.MediaBroker.toggleMute(), 
                description: 'Mute/Unmute' 
            });
            this.bindings.set('ArrowLeft', { 
                action: () => window.ZENITH_CORE.MediaBroker.seekTo(window.ZENITH_CORE.MediaBroker.getCurrentTime() - 10), 
                description: 'Seek backward 10s' 
            });
            this.bindings.set('ArrowRight', { 
                action: () => window.ZENITH_CORE.MediaBroker.seekTo(window.ZENITH_CORE.MediaBroker.getCurrentTime() + 10), 
                description: 'Seek forward 10s' 
            });

            // Playlist mappings (using Shift modifier to avoid collision with seeking)
            this.bindings.set('Shift+ArrowLeft', { 
                action: () => { if (window.ZENITH_CORE.PlaylistManager) window.ZENITH_CORE.PlaylistManager.prev(); }, 
                description: 'Previous Track' 
            });
            this.bindings.set('Shift+ArrowRight', { 
                action: () => { if (window.ZENITH_CORE.PlaylistManager) window.ZENITH_CORE.PlaylistManager.next(); }, 
                description: 'Next Track' 
            });

            // UI mappings
            this.bindings.set('KeyF', { 
                action: () => {
                    const ui = window.ZENITH_CORE.store.getState().ui;
                    window.ZENITH_CORE.store.dispatch({
                        type: 'TOGGLE_FULL_PLAYER',
                        payload: !ui.fullPlayerOpen
                    });
                }, 
                description: 'Toggle Full Player' 
            });
        }
        ...
        bindEvents() {
            window.addEventListener('keydown', (e) => {
                ...
                // Do not trigger shortcuts if user is typing in an input field or text area
                const activeTag = document.activeElement ? document.activeElement.tagName : '';
                if (activeTag === 'INPUT' || activeTag === 'TEXTAREA' || activeTag === 'YT-FORMATTED-STRING') {
                    return;
                }

                // Construct key string (e.g., 'Shift+ArrowLeft' or just 'Space')
                let keyCombo = '';
                if (e.ctrlKey || e.metaKey) keyCombo += 'Ctrl+'; // Optional expansion
                if (e.shiftKey) keyCombo += 'Shift+';
                
                // Use code instead of key to avoid language layout issues
                keyCombo += e.code; 

                // Special exception to prevent page scroll on Space if it acts as Play/Pause
                if (e.code === 'Space') {
                    e.preventDefault(); 
                }

                if (this.bindings.has(keyCombo)) {
                    this.logger.info('ShortcutManager', `Executing shortcut: ${this.bindings.get(keyCombo).description}`);
                    this.bindings.get(keyCombo).action();
                }
            });
        }
    ...
    window.ZENITH_CORE.ShortcutManager = ShortcutManager;
    // LINE COUNT TRACKER: FINAL FOR PART 13 (Line ~3998)
    // ================================================================================================================
    // MODULE 35: EQUALIZER (EQ) DSP CONTROLLER
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 14 (Line ~3999)
    class EQController {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.audioCtx = null;
            ...
            // Default 10-band frequencies, easily expandable to 15-band for advanced tuning
            this.frequencies = [31.5, 63, 125, 250, 500, 1000, 2000, 4000, 8000, 16000];
            this.gains = new Array(this.frequencies.length).fill(0); // Default flat
        }

        /**
         * Hooks into the existing AudioContext from the AudioVisualizerEngine.
         * Routing: Source -> EQ -> Visualizer Analyzer -> Destination
         */
        attach(audioContext, sourceNode, destinationNode) {
            if (!audioContext || !sourceNode) {
                this.logger.error('EQController', 'Failed to attach: Invalid AudioContext or SourceNode.');
                return false;
            }

            this.audioCtx = audioContext;
            this.sourceNode = sourceNode;
            this.outputNode = destinationNode || this.audioCtx.destination;

            this.buildFilterNetwork();
            this.logger.info('EQController', 'DSP Equalizer network successfully attached to audio routing.');
            return true;
        }

        buildFilterNetwork() {
            // Disconnect existing routing to insert EQ
            this.sourceNode.disconnect();
            ...
            this.filters = this.frequencies.map((freq, index) => {
                ...
                if (index === 0) {
                    ...
                    filter.type = 'peaking';
                    filter.Q.value = 1.41; // Standard Q factor for 1-octave bandwidth
                }
            ...
            // Connect the final filter to the output node
            if (this.isEnabled) {
                this.filters[this.filters.length - 1].connect(this.outputNode);
            } else {
                // If disabled, bypass the EQ entirely
        ...
        setBandGain(index, dbGain) {
            ...
            // Clamp gain between -15dB and +15dB
            const clampedGain = Math.max(-15, Math.min(15, dbGain));
            this.gains[index] = clampedGain;
            
            if (this.filters[index]) {
                // Smooth transition to avoid audio popping
                const currentTime = this.audioCtx.currentTime;
                this.filters[index].gain.setTargetAtTime(clampedGain, currentTime, 0.05);
            }

            // Sync with global store
            window.ZENITH_CORE.store.dispatch({
                type: 'UPDATE_EQ_SETTINGS',
                payload: { gains: [...this.gains] }
            });
        }

        setPreset(presetGains) {
            if (!Array.isArray(presetGains) || presetGains.length !== this.frequencies.length) {
                this.logger.warn('EQController', 'Invalid EQ preset provided. Length mismatch.');
                return;
            }
            presetGains.forEach((gain, index) => this.setBandGain(index, gain));
            this.logger.info('EQController', 'Applied new EQ preset.');
        }
        ...
        toggle(state) {
            ...
            if (this.isEnabled) {
                ...
                this.filters[this.filters.length - 1].connect(this.outputNode);
                this.logger.info('EQController', 'Equalizer bypassed: FALSE.');
            } else {
                // Bypass filters
                this.sourceNode.connect(this.outputNode);
                this.logger.info('EQController', 'Equalizer bypassed: TRUE.');
            }
        }
    }
    window.ZENITH_CORE.EQController = EQController;
    // LINE COUNT TRACKER: CURRENT ~4095

    // ================================================================================================================
    // MODULE 36: AUDIO SPATIALIZER (STEREO WIDENING & HAAS EFFECT)
    // ================================================================================================================
    class SpatializerEngine {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.audioCtx = null;
            this.inputNode = null;
            ...
            this.isEnabled = false;
            this.delayTimeMs = 15; // 10-25ms is ideal for the Haas effect (widening without echo)
            this.mixAmount = 0.5; // 0.0 (Dry) to 1.0 (Wet)
        ...
        attach(audioContext, sourceNode, destinationNode) {
            ...
            this.logger.info('Spatializer', 'Stereo enhancement network attached.');
            return true;
        }

        buildSpatialNetwork() {
            // Create nodes
            this.nodes.splitter = this.audioCtx.createChannelSplitter(2);
            this.nodes.merger = this.audioCtx.createChannelMerger(2);
            
            this.nodes.delayLeft = this.audioCtx.createDelay();
            this.nodes.delayRight = this.audioCtx.createDelay();
            
            this.nodes.dryGain = this.audioCtx.createGain();
            this.nodes.wetGain = this.audioCtx.createGain();

            // Set default parameters
            this.nodes.delayLeft.delayTime.value = 0; // Left stays prompt
            this.nodes.delayRight.delayTime.value = this.delayTimeMs / 1000.0; // Right is delayed
            
            this.updateMix(this.mixAmount);

            // Routing
            this.inputNode.connect(this.nodes.splitter);
            this.inputNode.connect(this.nodes.dryGain);
            
            // Left channel -> No delay
            this.nodes.splitter.connect(this.nodes.delayLeft, 0);
            this.nodes.delayLeft.connect(this.nodes.merger, 0, 0);
            
            // Right channel -> Haas Delay
            this.nodes.splitter.connect(this.nodes.delayRight, 1);
            this.nodes.delayRight.connect(this.nodes.merger, 0, 1);
            
            // Merge wet signal
            this.nodes.merger.connect(this.nodes.wetGain);

            if (this.isEnabled) {
                this.nodes.dryGain.connect(this.outputNode);
                this.nodes.wetGain.connect(this.outputNode);
            }
        }

        updateMix(amount) {
            this.mixAmount = Math.max(0, Math.min(1, amount));
            if (this.nodes.wetGain && this.nodes.dryGain) {
                // Constant power panning curve for mixing wet/dry
                this.nodes.wetGain.gain.value = Math.sin(this.mixAmount * Math.PI / 2);
                this.nodes.dryGain.gain.value = Math.cos(this.mixAmount * Math.PI / 2);
            }
        }

        setDelay(ms) {
            this.delayTimeMs = Math.max(1, Math.min(40, ms));
            if (this.nodes.delayRight) {
                this.nodes.delayRight.delayTime.value = this.delayTimeMs / 1000.0;
            }
        }

        toggle(state) {
            this.isEnabled = state;
            if (!this.nodes.dryGain || !this.nodes.wetGain) return;

            this.nodes.dryGain.disconnect();
            this.nodes.wetGain.disconnect();

            if (this.isEnabled) {
                this.nodes.dryGain.connect(this.outputNode);
                this.nodes.wetGain.connect(this.outputNode);
                this.logger.info('Spatializer', 'Stereo Widener activated.');
            } else {
                // If disabled, just pass dry signal at full volume
                this.nodes.dryGain.gain.value = 1.0;
                this.nodes.dryGain.connect(this.outputNode);
                this.logger.info('Spatializer', 'Stereo Widener bypassed.');
            }
        }
    }
    window.ZENITH_CORE.SpatializerEngine = SpatializerEngine;
    // LINE COUNT TRACKER: FINAL FOR PART 14 (Line ~4185)
    // ================================================================================================================
    // MODULE 37: DOM INJECTOR & LIFECYCLE MANAGER
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 15 (Line ~4186)
    class LifecycleManager {
        constructor() {
            ...
            this.logger.info('Lifecycle', 'Lifecycle Manager instantiated. Awaiting document readiness.');
        }

        /**
         * Begins the boot sequence. Checks if the host page (YouTube Music) is ready.
         */
        boot() {
            if (this.isInitialized) return;

            if (document.readyState === 'complete' || document.readyState === 'interactive') {
                this.checkHostDependencies();
            } else {
                window.addEventListener('DOMContentLoaded', () => this.checkHostDependencies());
                window.addEventListener('load', () => this.checkHostDependencies());
            }
        }

        /**
         * Validates that the native YouTube Music video element and app framework exist.
         */
        checkHostDependencies() {
            ...
            if (ytApp && videoEl) {
                this.logger.info('Lifecycle', 'Host dependencies found. Proceeding with subsystem initialization.');
                this.initializeSubsystems(videoEl);
            } else {
                if (this.retryCount < this.maxRetries) {
                    this.retryCount++;
                    this.logger.warn('Lifecycle', `Host elements missing. Retrying... (${this.retryCount}/${this.maxRetries})`);
                    setTimeout(() => this.checkHostDependencies(), 1000);
                } else {
                    this.logger.error('Lifecycle', 'Failed to find host dependencies after maximum retries. Boot aborted.');
                }
            }
        }

        /**
         * Boots all Zenith modules in the strict dependency order.
         * @param {HTMLVideoElement} videoElement - The native media element.
         */
        initializeSubsystems(videoElement) {
            try {
                this.logger.info('Lifecycle', '--- STARTING SUBSYSTEM BOOT SEQUENCE ---');

                // 1. Storage & Settings
                window.ZENITH_CORE.storage = new window.ZENITH_CORE.StorageAdapter();
                const savedState = window.ZENITH_CORE.storage.load('APP_STATE', {});

                // 2. State Management (Redux-lite Store)
                // Assuming createStore exists in earlier modules, we sync it here
                if (savedState.eq) {
                    window.ZENITH_CORE.store.dispatch({ type: 'UPDATE_EQ_SETTINGS', payload: savedState.eq });
                }

                // 3. Media Broker & Hardware Bridging
                window.ZENITH_CORE.mediaBroker = new window.ZENITH_CORE.MediaBroker(videoElement);
                window.ZENITH_CORE.mediaSession = new window.ZENITH_CORE.MediaSessionBridge();
                window.ZENITH_CORE.shortcuts = new window.ZENITH_CORE.ShortcutManager();

                // 4. Playlist & Gapless Engine
                window.ZENITH_CORE.playlistManager = new window.ZENITH_CORE.PlaylistManager();
                window.ZENITH_CORE.gaplessFetcher = new window.ZENITH_CORE.GaplessPreFetcher();

                // 5. Lyrics Engine
                window.ZENITH_CORE.lyricsEngine = new window.ZENITH_CORE.LyricsSyncEngine();

                // 6. Audio DSP (Visualizer, EQ, Spatializer)
                window.ZENITH_CORE.audioEngine = new window.ZENITH_CORE.AudioVisualizerEngine();
                const audioCtx = window.ZENITH_CORE.audioEngine.getAudioContext();
                const sourceNode = window.ZENITH_CORE.audioEngine.getSourceNode();
                
                if (audioCtx && sourceNode) {
                    window.ZENITH_CORE.eqController = new window.ZENITH_CORE.EQController();
                    window.ZENITH_CORE.spatializer = new window.ZENITH_CORE.SpatializerEngine();
                    
                    // Routing: Source -> EQ -> Spatializer -> Visualizer -> Destination
                    window.ZENITH_CORE.eqController.attach(audioCtx, sourceNode, null);
                    // Additional routing logic would link these nodes dynamically based on enabled states
                }

                // 7. Inject DOM Elements
                this.injectDOM();

                this.isInitialized = true;
                this.logger.info('Lifecycle', '--- SUBSYSTEM BOOT SEQUENCE COMPLETE ---');
                
                // Dispatch global ready event
                window.ZENITH_CORE.bus.emit('SYSTEM_READY', { timestamp: Date.now() });

            } catch (error) {
                this.logger.error('Lifecycle', 'Fatal error during subsystem initialization.', error);
            }
        }

        /**
         * Mounts the Zenith UI container into the YouTube Music DOM.
         */
        injectDOM() {
            this.logger.info('Lifecycle', 'Injecting Zenith UI containers...');

            // Check if already injected
            if (document.getElementById('zenith-root')) {
                this.logger.warn('Lifecycle', 'Zenith root already exists in DOM. Skipping injection.');
                return;
            }

            const rootContainer = document.createElement('div');
            rootContainer.id = 'zenith-root';
            
            // We append this to the body, but use z-index and absolute positioning 
            // to float above the YTM interface.
            document.body.appendChild(rootContainer);

            // Optional: Inject custom global stylesheet
            const styleEl = document.createElement('style');
            styleEl.id = 'zenith-global-styles';
            // CSS will be hydrated by the UI Renderer module
            document.head.appendChild(styleEl);

            this.logger.info('Lifecycle', 'DOM injection successful.');
        }

        /**
         * Gracefully tears down the extension.
         */
        shutdown() {
            if (!this.isInitialized) return;
            
            this.logger.info('Lifecycle', 'Initiating shutdown sequence...');
            
            // Disconnect observers, clear intervals, detach audio nodes
            if (window.ZENITH_CORE.audioEngine) window.ZENITH_CORE.audioEngine.disconnect();
            if (window.ZENITH_CORE.shortcuts) window.ZENITH_CORE.shortcuts.disable();
            
            const root = document.getElementById('zenith-root');
            if (root) root.remove();

            const styles = document.getElementById('zenith-global-styles');
            if (styles) styles.remove();

            this.isInitialized = false;
            window.ZENITH_CORE.bus.emit('SYSTEM_SHUTDOWN', {});
            this.logger.info('Lifecycle', 'Shutdown complete.');
        }
    }
    window.ZENITH_CORE.LifecycleManager = LifecycleManager;

    // ================================================================================================================
    // BOOTSTRAP EXECUTION
    // ================================================================================================================
    // This immediately invokes the lifecycle manager when the script is parsed.
    (function bootstrap() {
        const lifecycle = new window.ZENITH_CORE.LifecycleManager();
        window.ZENITH_CORE.lifecycle = lifecycle;
        lifecycle.boot();
    })();
    // LINE COUNT TRACKER: FINAL FOR PART 15 (Line ~4325)
    // ================================================================================================================
    // MODULE 38: THEME ENGINE & DYNAMIC CSS GENERATOR
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 16 (Line ~4326)
    class ThemeEngine {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.styleElement = document.getElementById('zenith-global-styles') || this.createStyleNode();
            ...
            this.defaultTheme = {
                ...
                primary: '#ffffff',
                secondary: '#b3b3b3',
                accent: '#1db954', // Default green, overridden by album art
                error: '#e22134'
            };
        ...
        createStyleNode() {
            ...
            el.id = 'zenith-global-styles';
            document.head.appendChild(el);
            return el;
        }

        /**
         * Extracts dominant color from album art and updates the CSS variables.
         * Falls back to default accent if extraction fails.
         */
        applyDynamicAccent(imageUrl) {
            if (!imageUrl) {
                this.updateCSSVariables(this.defaultTheme.accent);
                return;
            }

            // A simplified color extraction (in a full build, this would use a canvas to analyze pixels)
            this.logger.info('ThemeEngine', `Extracting color profile from: ${imageUrl}`);
            ...
            img.onload = () => {
                ...
                try {
                    // Sample the center pixel for a quick accent color
                    const pixel = ctx.getImageData(canvas.width / 2, canvas.height / 2, 1, 1).data;
                    const rgb = `rgb(${pixel[0]}, ${pixel[1]}, ${pixel[2]})`;
                    this.updateCSSVariables(rgb);
                } catch (e) {
                    this.logger.warn('ThemeEngine', 'CORS error extracting image color. Using default.');
                    this.updateCSSVariables(this.defaultTheme.accent);
                }
            };
            img.onerror = () => this.updateCSSVariables(this.defaultTheme.accent);
            img.src = imageUrl;
        }

        updateCSSVariables(accentColor) {
            this.currentTheme.accent = accentColor;
            
            const cssVars = `
                :root {
                    --z-bg: ${this.currentTheme.background};
                    --z-surface: ${this.currentTheme.surface};
                    --z-text-primary: ${this.currentTheme.primary};
                    --z-text-secondary: ${this.currentTheme.secondary};
                    --z-accent: ${this.currentTheme.accent};
                    --z-error: ${this.currentTheme.error};
                    ...
                    --z-shadow-elevation-1: 0 4px 6px rgba(0,0,0,0.3);
                    --z-shadow-elevation-2: 0 10px 20px rgba(0,0,0,0.5);
                    
                    --z-transition-fast: 150ms ease-in-out;
                    --z-transition-smooth: 300ms cubic-bezier(0.4, 0, 0.2, 1);
                }
            `;
            
            // We append this to the existing stylesheet without overwriting component styles
            this.styleElement.textContent = cssVars + (this.componentCSS || '');
            this.logger.info('ThemeEngine', `CSS Variables updated. Accent: ${accentColor}`);
        }

        injectComponentCSS(cssString) {
            this.componentCSS = cssString;
            this.updateCSSVariables(this.currentTheme.accent);
        }
    }
    window.ZENITH_CORE.ThemeEngine = ThemeEngine;
    // LINE COUNT TRACKER: CURRENT ~4410

    // ================================================================================================================
    // MODULE 39: UI COMPONENT RENDERER
    // ================================================================================================================
    class UIRenderer {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.rootElement = document.getElementById('zenith-root');
        ...
        /**
         * Mounts a generated DOM tree to a specific container
         */
        mount(containerId, element) {
            const container = document.getElementById(containerId) || this.rootElement;
            if (!container) {
                this.logger.error('UIRenderer', `Mount target #${containerId} not found.`);
                return;
            }

            container.innerHTML = ''; // Clear existing
            container.appendChild(element);
            this.logger.info('UIRenderer', `Component mounted to #${containerId || 'zenith-root'}.`);
        }

        /**
         * Generates the baseline UI scaffold (Player bar, Modals, Fullscreen view)
         */
        renderScaffold() {
            const scaffold = this.createElement('div', { id: 'z-app-container', className: 'z-theme-dark' },
                this.createElement('div', { id: 'z-fullscreen-layer', className: 'z-hidden' }),
                this.createElement('div', { id: 'z-modal-layer', className: 'z-hidden' }),
                this.createElement('div', { id: 'z-mini-player-layer' }),
                this.createElement('div', { id: 'z-toast-container' })
            );

            this.mount(null, scaffold);
        }
        
        showToast(message, duration = 3000) {
            const toastContainer = document.getElementById('z-toast-container');
            if (!toastContainer) return;

            const toast = this.createElement('div', { className: 'z-toast z-toast-enter' }, message);
            toastContainer.appendChild(toast);

            // Trigger reflow for animation
            void toast.offsetWidth;
            toast.classList.remove('z-toast-enter');

            setTimeout(() => {
                toast.classList.add('z-toast-exit');
                setTimeout(() => toast.remove(), 300); // Wait for CSS transition
            }, duration);
        }
    }
    window.ZENITH_CORE.UIRenderer = UIRenderer;
    // LINE COUNT TRACKER: FINAL FOR PART 16 (Line ~4488)
    // ================================================================================================================
    // MODULE 40: CORE UI CSS HYDRATION
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 17 (Line ~4489)
    class CSSHydrator {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.themeEngine = new window.ZENITH_CORE.ThemeEngine();
            this.baseCSS = `
                /* Zenith Base Reset */
                #zenith-root * {
                    box-sizing: border-box;
                ...
                #zenith-root {
                    ...
                    height: 100vh;
                    z-index: 999999;
                    pointer-events: none; /* Let clicks pass through where no UI exists */
                    display: flex;
                    flex-direction: column;
                ...
                #z-mini-player-layer {
                    ...
                    align-items: center;
                    padding: 0 16px;
                    box-shadow: var(--z-shadow-elevation-2);
                    transition: transform var(--z-transition-smooth);
                    transform: translateY(100%); /* Hidden by default */
                }

                #z-mini-player-layer.z-active {
                    transform: translateY(0);
                }
                ...
                #z-fullscreen-layer {
                    ...
                    opacity: 0;
                    visibility: hidden;
                    transition: opacity var(--z-transition-smooth), visibility var(--z-transition-smooth);
                    padding: 24px;
                }

                #z-fullscreen-layer.z-active {
                    opacity: 1;
                    visibility: visible;
                }
                ...
                .z-flex-center { display: flex; justify-content: center; align-items: center; }
                ...
                .z-toast {
                    ...
                    font-size: 14px;
                    font-weight: 500;
                    box-shadow: var(--z-shadow-elevation-1);
                    opacity: 1;
                    transition: opacity var(--z-transition-fast), transform var(--z-transition-fast);
                }

                .z-toast-enter { opacity: 0; transform: translateY(10px); }
                .z-toast-exit { opacity: 0; transform: translateY(-10px); }
            `;
        }

        hydrate() {
            this.themeEngine.injectComponentCSS(this.baseCSS);
            this.logger.info('CSSHydrator', 'Core structural CSS injected and hydrated.');
        }
    }
    window.ZENITH_CORE.CSSHydrator = CSSHydrator;
    // LINE COUNT TRACKER: CURRENT ~4575

    // ================================================================================================================
    // MODULE 41: MAIN UI LAYOUT BUILDER
    // ================================================================================================================
    class LayoutBuilder {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.renderer = new window.ZENITH_CORE.UIRenderer();
            this.cssHydrator = new window.ZENITH_CORE.CSSHydrator();
        }

        build() {
            this.logger.info('LayoutBuilder', 'Constructing Virtual DOM architecture...');
            
            // 1. Inject Styles
            this.cssHydrator.hydrate();

            // 2. Build Scaffold
            this.renderer.renderScaffold();

            // 3. Construct Mini Player
            this.buildMiniPlayer();

            // 4. Construct Full Player (hidden initially)
            this.buildFullPlayer();
            
            this.logger.info('LayoutBuilder', 'Virtual DOM construction complete.');
        }

        buildMiniPlayer() {
            const container = document.getElementById('z-mini-player-layer');
            if (!container) return;

            // Simplified Mockup Structure for Mini Player
            const art = this.renderer.createElement('div', { className: 'z-mini-art', style: 'width: 48px; height: 48px; background: #333; border-radius: var(--z-radius-sm); margin-right: 16px;' });
            const metadata = this.renderer.createElement('div', { className: 'z-mini-meta', style: 'flex-grow: 1; display: flex; flex-direction: column;' },
                this.renderer.createElement('span', { className: 'z-title', style: 'color: var(--z-text-primary); font-weight: 500; font-size: 14px;' }, 'Not Playing'),
                this.renderer.createElement('span', { className: 'z-artist', style: 'color: var(--z-text-secondary); font-size: 12px;' }, '--')
            );
            
            const controls = this.renderer.createElement('div', { className: 'z-mini-controls', style: 'display: flex; gap: 16px; align-items: center;' },
                this.renderer.createElement('button', { className: 'z-btn-prev', style: 'background: none; border: none; color: var(--z-text-primary); cursor: pointer;' }, '⏮'),
                this.renderer.createElement('button', { className: 'z-btn-play', style: 'background: none; border: none; color: var(--z-text-primary); font-size: 24px; cursor: pointer;' }, '▶'),
                this.renderer.createElement('button', { className: 'z-btn-next', style: 'background: none; border: none; color: var(--z-text-primary); cursor: pointer;' }, '⏭')
            );
            ...
            // Activate it for preview
            container.classList.add('z-active');
        }

        buildFullPlayer() {
            const container = document.getElementById('z-fullscreen-layer');
            if (!container) return;
            
            const closeBtn = this.renderer.createElement('button', { 
                style: 'position: absolute; top: 24px; left: 24px; background: none; border: none; color: white; font-size: 24px; cursor: pointer;',
                onClick: () => container.classList.remove('z-active')
            }, '⌄');

            const placeholder = this.renderer.createElement('div', { style: 'color: white; margin: auto; font-size: 24px;' }, 'Zenith Full Player Framework Ready');

            container.appendChild(closeBtn);
            container.appendChild(placeholder);
        }
    }
    window.ZENITH_CORE.LayoutBuilder = LayoutBuilder;
    
    // Auto-execute layout generation if lifecycle is ready
    if (window.ZENITH_CORE.lifecycle && window.ZENITH_CORE.lifecycle.isInitialized) {
        const builder = new window.ZENITH_CORE.LayoutBuilder();
        builder.build();
    }
    // LINE COUNT TRACKER: FINAL FOR PART 17 (Line ~4645)
    // ================================================================================================================
    // MODULE 42: UI EVENT CONTROLLER & STATE BINDING
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 18 (Line ~4646)
    class UIEventController {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.bus = window.ZENITH_CORE.bus;
            this.mediaBroker = window.ZENITH_CORE.mediaBroker;
            this.renderer = window.ZENITH_CORE.UIRenderer ? new window.ZENITH_CORE.UIRenderer() : null;
            
            this.elements = {
                miniPlayer: document.getElementById('z-mini-player-layer'),
                fullPlayer: document.getElementById('z-fullscreen-layer'),
                title: document.querySelector('#z-mini-player-layer .z-title'),
                artist: document.querySelector('#z-mini-player-layer .z-artist'),
                art: document.querySelector('#z-mini-player-layer .z-mini-art'),
                btnPlay: document.querySelector('#z-mini-player-layer .z-btn-play'),
                btnPrev: document.querySelector('#z-mini-player-layer .z-btn-prev'),
                btnNext: document.querySelector('#z-mini-player-layer .z-btn-next')
            };

            this.bindEvents();
            this.logger.info('UIEventController', 'UI elements successfully bound to state and media broker.');
        }

        bindEvents() {
            // 1. Bind UI Interactions to Media Broker
            if (this.elements.btnPlay) {
                this.elements.btnPlay.addEventListener('click', (e) => {
                    e.stopPropagation();
                    this.mediaBroker.togglePlay();
                });
            }
            ...
            if (this.elements.btnNext) {
                this.elements.btnNext.addEventListener('click', (e) => {
                    e.stopPropagation();
                    this.mediaBroker.next();
                });
            }

            // Expand to full player on mini-player click
            if (this.elements.miniPlayer && this.elements.fullPlayer) {
                this.elements.miniPlayer.addEventListener('click', () => {
                    this.elements.fullPlayer.classList.add('z-active');
                });
            }

            // 2. Bind State Changes to UI
            this.bus.on('MEDIA_STATE_CHANGED', (state) => this.updatePlayState(state.isPlaying));
            this.bus.on('TRACK_CHANGED', (trackInfo) => this.updateMetadata(trackInfo));
        }

        updatePlayState(isPlaying) {
            if (!this.elements.btnPlay) return;
            this.elements.btnPlay.textContent = isPlaying ? '⏸' : '▶';
        }

        updateMetadata(trackInfo) {
            if (!trackInfo) return;

            if (this.elements.title) this.elements.title.textContent = trackInfo.title || 'Unknown Title';
            if (this.elements.artist) this.elements.artist.textContent = trackInfo.artist || 'Unknown Artist';
            
            if (this.elements.art && trackInfo.coverUrl) {
                this.elements.art.style.background = `url(${trackInfo.coverUrl}) center/cover no-repeat`;
                
                // Trigger theme extraction if ThemeEngine exists
                if (window.ZENITH_CORE.ThemeEngine) {
                    const theme = new window.ZENITH_CORE.ThemeEngine();
                    theme.applyDynamicAccent(trackInfo.coverUrl);
                }
            }
        }
    }
    window.ZENITH_CORE.UIEventController = UIEventController;
    // LINE COUNT TRACKER: CURRENT ~4720

    // ================================================================================================================
    // MODULE 43: NATIVE DOM SANITIZER (UI HIJACKER)
    // ================================================================================================================
    class NativeSanitizer {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.observer = null;
            this.isActive = false;
            
            // CSS Selectors for native YouTube Music UI elements that must be hidden
            this.targetSelectors = [
                'ytmusic-player-bar',
                'ytmusic-nav-bar',
                '#layout',
        ...
        /**
         * Aggressively hides native UI using a highly specific injected stylesheet.
         * We use CSS over DOM removal to prevent React/Polymer from throwing fatal errors.
         */
        applySanitizationCSS() {
            if (this.styleNode) return;
            
            this.styleNode = document.createElement('style');
            this.styleNode.id = 'zenith-sanitizer-styles';
            
            const hiddenRules = this.targetSelectors.map(selector => 
                `${selector} { display: none !important; opacity: 0 !important; pointer-events: none !important; }`
            ).join('\n');
            
            // We MUST preserve the video element and its immediate container for playback to function
            const preserveRules = `
                ytmusic-player { position: absolute; top: -9999px; left: -9999px; width: 1px; height: 1px; visibility: hidden; }
                video { display: block !important; width: 1px !important; height: 1px !important; }
            `;

            this.styleNode.textContent = hiddenRules + '\n' + preserveRules;
            document.head.appendChild(this.styleNode);
            
            this.logger.info('NativeSanitizer', 'CSS suppression rules applied to native interface.');
        }
        ...
        start() {
            ...
            this.applySanitizationCSS();

            // Fallback: MutationObserver to catch shadow-DOM re-renders or dynamically injected native elements
            this.observer = new MutationObserver((mutations) => {
                for (const mutation of mutations) {
                    if (mutation.addedNodes.length) {
                        this.enforceSuppression();
                    }
                }
            });

            this.observer.observe(document.body, { childList: true, subtree: true });
            this.logger.info('NativeSanitizer', 'MutationObserver active. Native UI locked down.');
        }

        enforceSuppression() {
            // Backup enforcement for rogue elements that bypass the CSS
            const ads = document.querySelectorAll('ytmusic-player-overlay-autoplay, .ytp-ad-module');
            ads.forEach(ad => {
                if (ad.style.display !== 'none') ad.style.display = 'none';
            });
        }
        ...
        stop() {
            ...
            this.logger.info('NativeSanitizer', 'Sanitizer stopped. Native UI restored.');
        }
    }
    window.ZENITH_CORE.NativeSanitizer = NativeSanitizer;

    // Boot UI modules if lifecycle is complete
    if (window.ZENITH_CORE.lifecycle && window.ZENITH_CORE.lifecycle.isInitialized) {
        window.ZENITH_CORE.uiController = new window.ZENITH_CORE.UIEventController();
        window.ZENITH_CORE.sanitizer = new window.ZENITH_CORE.NativeSanitizer();
        window.ZENITH_CORE.sanitizer.start();
    }
    // LINE COUNT TRACKER: FINAL FOR PART 18 (Line ~4795)
    // ================================================================================================================
    // MODULE 44: TELEMETRY INTERCEPTOR & ANALYTICS BLOCKER
    // ================================================================================================================
    // LINE COUNT TRACKER: START PART 19 (Line ~4796)
    class TelemetryInterceptor {
        constructor() {
            this.logger = window.ZENITH_CORE.Logger;
            this.isActive = false;
            
            // Substrings in request URLs that identify analytics, tracking, and error reporting endpoints
            ...
            // Store original native functions to restore them if needed
            this.natives = {
                fetch: window.fetch,
        ...
        /**
         * Determines if a given URL matches any of the blacklisted telemetry endpoints.
         * @param {string} url - The URL to check.
         * @returns {boolean} True if the request should be blocked.
         */
        isBlacklisted(url) {
            if (!url || typeof url !== 'string') return false;
            return this.blacklistedEndpoints.some(endpoint => url.includes(endpoint));
        }

        /**
         * Injects wrappers around native network request APIs to intercept and drop telemetry.
         */
        start() {
            if (this.isActive) return;
            this.isActive = true;
            this.logger.info('TelemetryInterceptor', 'Deploying network hooks for telemetry suppression...');

            this.hijackFetch();
            this.hijackXHR();
            this.hijackSendBeacon();

            this.logger.info('TelemetryInterceptor', 'Telemetry successfully isolated and blocked.');
        }

        /**
         * Restores native network APIs.
         */
        stop() {
            if (!this.isActive) return;
            this.isActive = false;

            window.fetch = this.natives.fetch;
            window.XMLHttpRequest.prototype.open = this.natives.xhrOpen;
            window.XMLHttpRequest.prototype.send = this.natives.xhrSend;
            navigator.sendBeacon = this.natives.sendBeacon;

            this.logger.info('TelemetryInterceptor', 'Network hooks removed. Telemetry restored.');
        }

        /**
         * Wraps the native fetch API.
         * Resolves blacklisted requests immediately with an empty 200 OK response 
         * to prevent the host application from throwing unhandled promise rejections.
         */
        hijackFetch() {
            const self = this;
            window.fetch = async function(...args) {
                ...
                if (self.isBlacklisted(requestUrl)) {
                    // self.logger.debug('TelemetryInterceptor', `Blocked fetch: ${requestUrl}`);
                    return new Response(JSON.stringify({ success: true, zenith_intercepted: true }), {
                        status: 200,
                        statusText: 'OK',
                ...
                // Proceed with standard fetch for music data, images, etc.
                return self.natives.fetch.apply(this, args);
            };
        }

        /**
         * Wraps the native XMLHttpRequest API.
         * Aborts blacklisted requests silently while faking a successful readyState progression.
         */
        hijackXHR() {
            const self = this;

            window.XMLHttpRequest.prototype.open = function(method, url, ...rest) {
                this._zenith_url = url;
                this._zenith_blocked = self.isBlacklisted(url);
                return self.natives.xhrOpen.call(this, method, url, ...rest);
            };

            window.XMLHttpRequest.prototype.send = function(body) {
                if (this._zenith_blocked) {
                    // self.logger.debug('TelemetryInterceptor', `Blocked XHR: ${this._zenith_url}`);
                    
                    // Fake the successful completion of the XHR to keep the host app happy
                    ...
                    return; // Do not call native send
                }

                return self.natives.xhrSend.call(this, body);
            };
        }

        /**
         * Wraps navigator.sendBeacon, commonly used by modern apps for unload telemetry.
         */
        hijackSendBeacon() {
            const self = this;
            navigator.sendBeacon = function(url, data) {
                if (self.isBlacklisted(url)) {
                    // self.logger.debug('TelemetryInterceptor', `Blocked Beacon: ${url}`);
                    return true; // Browser expects a boolean indicating successful queuing
                }
                return self.natives.sendBeacon.call(this, url, data);
            };
        }
    }
    window.ZENITH_CORE.TelemetryInterceptor = TelemetryInterceptor;

    // Boot telemetry blocker immediately if core exists
    if (window.ZENITH_CORE) {
        window.ZENITH_CORE.telemetryBlocker = new window.ZENITH_CORE.TelemetryInterceptor();
        window.ZENITH_CORE.telemetryBlocker.start();
    }
    // LINE COUNT TRACKER: FINAL FOR PART 19 (Line ~4915)
